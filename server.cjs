...
const loadAuth = async () => {
  ...
  if (!mod || !mod.auth) {
    throw new Error('Auth module did not export auth');
  }
  return mod.auth;
};
...
const serveStatic = async (req, res) => {
  ...
  if (pathname === '/' || pathname === '') {
    const html = await fs.promises.readFile(indexHtmlPath);
    res.statusCode = 200;
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.startsWith('/api/pesapal')) {
      const handler = await loadDefaultHandler('./api/pesapal/[...all].js');
      await handler(req, res);
      return;
    }

    if (url.startsWith('/api/upload')) {
      const handler = await loadDefaultHandler('./api/upload/index.js');
      await handler(req, res);
      return;
    }

    if (url.startsWith('/api/books')) {
      const handler = await loadDefaultHandler('./api/books/index.js');
      await handler(req, res);
      return;
    }
